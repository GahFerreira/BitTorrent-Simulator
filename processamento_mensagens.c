...
void *processar_mensagens_recebidas(info_total_t *info_total)
{
    ...
    while (info_total->info_compartilhada->finalizar_execucao == false)
    {
        ...
        // Segundo passo - O usuário checa se há alguma nova solicitação de arquivo.
        checar_solicitacoes_arquivo(info_total, solicitacoes_arquivo, id_usuario);

        // Terceiro passo - O usuário checa se há alguma nova mensagem de completude de arquivo.
        checar_mensagem_arquivo_completo(info_total, arquivo_completo, id_usuario);

        // Roda 20 vezes por segundo.
        meu_sleep(50);
    }
...
{
    #if DEBUG >= 7
    printf("[DEBUG-7] Usuario %u checa novos usuarios conectados.\n\n", id_usuario+1);
    #endif
    ...
    if (novo_usuario_conectado != NULL)
    {
        ...
        */
        ...
        /*
            ...
            Implementação acabou ficando descontinuada: solicitações ficam salvas para quando o usuário entrar.
        */
...
{
    #if DEBUG >= 7
    printf("[DEBUG-7] Usuario %u checa solicitacoes de arquivo.\n\n", id_usuario+1);
    #endif
    ...
    if (solicitacao_arquivo != NULL)
    {
        ...
        if (estado_arquivo == COMPLETO)
        {
            ...
            #if DEBUG >= 5
            printf("[DEBUG-5] Usuario %u adiciona tarefa <usuario: %u, arquivo: %u> em sua lista de tarefas.\n\n", id_usuario+1, solicitacao_arquivo->id_usuario+1, solicitacao_arquivo->id_arquivo+1);
            #endif
        ...
        free((par_usuario_arquivo_t *) solicitacao_arquivo);
    }
}
...
{
    #if DEBUG >= 7
    printf("[DEBUG-7] Usuario %u checa novas mensagens de completude de arquivos.\n\n", id_usuario+1);
    #endif
